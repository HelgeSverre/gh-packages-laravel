...
'providers' => [
    ...
    Jmrashed\TwoFactorAuth\TwoFactorServiceProvider::class,
],
```
...
Included in the package are custom views and helper functions dedicated to managing Two-Factor Authentication (2FA) during login attempts and settings management. These resources facilitate the implementation of 2FA without the need for additional middleware or new guards. However, advanced users have the option to customize the authentication process manually if desired.
...
### Managing Two-Factor Authentication Methods

Users can manage their Two-Factor Authentication (2FA) methods (TOTP, SMS, Email) through a dedicated settings page.

To access the 2FA settings, navigate to the `/two-factor-settings` route in your application. This page allows users to:

-   **Enable/Disable TOTP**: Generate a QR code and secret key for authenticator apps, confirm the setup, or disable TOTP.
-   **Enable/Disable SMS 2FA**: Enter and verify a phone number, send verification codes, and enable/disable SMS authentication.
-   **Enable/Disable Email 2FA**: Enable email-based 2FA, send verification codes to their registered email, and disable it.

Here's an example of how you might link to the settings page:

```blade
<a href="{{ route('two-factor.settings') }}">Manage 2FA Settings</a>
```

#### Enabling TOTP (Authenticator App)

To enable TOTP, the user needs to synchronize a Shared Secret with their Authenticator app.

> **Tip:**
>
> Popular free Authenticator apps include iOS Authenticator, FreeOTP, Authy, and Google Authenticator, among others.

On the 2FA settings page, when enabling TOTP, a QR code and a secret key will be displayed. The user should scan the QR code or manually enter the secret key into their authenticator app. After setting up, they will be prompted to enter a code from their app to confirm and activate TOTP.

#### Enabling SMS 2FA

To enable SMS 2FA, the user must provide and verify their phone number.

On the 2FA settings page, the user can enter their phone number. After saving, a verification code will be sent to the provided number. The user must then enter this code to verify their phone number and activate SMS 2FA.

#### Enabling Email 2FA

To enable Email 2FA, the user's registered email address will be used.

On the 2FA settings page, the user can enable Email 2FA. A verification code will be sent to their registered email address. The user must then enter this code to verify and activate Email 2FA.
